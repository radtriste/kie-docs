[id='proc_kogito-app-examples-running_{context}']
...
For this procedure, use the https://github.com/kiegroup/kogito-examples/tree/stable/jbpm-quarkus-example[`jbpm-quarkus-example`] or https://github.com/kiegroup/kogito-examples/tree/stable/jbpm-springboot-example[`jbpm-springboot-example`] application. You can follow similar steps with the other {PRODUCT} examples on Quarkus or Spring Boot.

In the `jbpm-quarkus-example` and `jbpm-springboot-example` applications, the `orders.bpmn2` process describes the steps that need to be followed when ordering items. The process includes a script task for writing debug information and a call activity for invoking a sub-process, using a custom `Order` data object.
...
. In a command terminal, navigate to the extracted `kogito-examples-__RELEASE__/jbpm-__PLATFORM__-example` folder, and enter one of the following commands to build and run the example.
+
--
