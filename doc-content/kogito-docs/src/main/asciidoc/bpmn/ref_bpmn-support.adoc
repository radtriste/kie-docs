...
*<@Cristiano, in the following table, see Tasks > Custom (work item). We say in the runtimes wiki that Kogito supports it so I've included it, but I don't find any more details about how to actually configure customer tasks / work items and handlers, etc. to use them. In BA 7.x, we have a https://access.redhat.com/documentation/en-us/red_hat_process_automation_manager/7.7/html-single/custom_tasks_and_work_item_handlers_in_business_central/index[Custom tasks and work item handlers] doc that addresses that. What is the equivalent for Kogito in order to use custom tasks in processes?>*
