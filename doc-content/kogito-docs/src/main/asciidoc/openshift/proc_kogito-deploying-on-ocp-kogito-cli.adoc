...
. Extract the `{PRODUCT_INIT}-_RELEASE_` binary file:
+
--
* On Linux: In a command terminal, navigate to the directory where you downloaded the `{PRODUCT_INIT}-_RELEASE_` binary file and run the following command to extract the contents:
+
